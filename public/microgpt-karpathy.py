...
# Built-in fallback dataset used when remote names.txt cannot be loaded.
fallback_docs = [
...
if isinstance(names_text, str) and names_text.strip():
    docs = [line.strip() for line in names_text.splitlines() if line.strip()]
    dataset_name = "makemore names.txt"
else:
    docs = fallback_docs
    dataset_name = "fallback embedded list"
